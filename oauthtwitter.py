...
class OAuthApi(Api):
    ...
    def _FetchUrl(self,*args, **kwargs):
        '''A wrapper for FetchUrl, twitter server sucks so in order to be able
            avoid catching errors in one thousand places each time they are
            too busy, we wrap to repeat until they are back.
        '''
        wait_time = {502:2, 104:2, 503:10, 54:2}
        tries = 0
        while tries < 20: #Arbitrarity rules
            try:
                return self.__FetchUrl(*args, **kwargs)
            except HTTPError, httperr:
                #Yea, blocking, sue me
		if (type(httperr) is URLError) and (httperr.reason[0] in wait_time.keys()):
			tries += 1
                    	time.sleep(wait_time.get(httperr.reason[0]))
                elif httperr.code in wait_time.keys():
                    tries += 1
                    time.sleep(wait_time.get(httperr.code))
                else:
                    raise
            except URLError, urlerr:
		if urlerr.reason[0] in wait_time.keys():
                    tries += 1
                    time.sleep(wait_time.get(urlerr.reason[0]))
                else:
                    raise
